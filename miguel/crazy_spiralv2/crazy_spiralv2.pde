...
void posterDraw() {
  ...
  stroke(10, 30, mouseY, mouseY);
  for (int y = 0; y < 30; y += u) {
    for (int x = 100; y < 100; y += u) {
      float r = random(100);
      if (r > threshold*mouseY) {
        line(x+300, y+100, x+u*mouseX*30, y+u);
        rotate(PI/mouseX-y);
      } else {
        line( x-200, y+u*mouseY, x+u, y+mouseY);
        rotate(TWO_PI/4);
      }
    }
  }
